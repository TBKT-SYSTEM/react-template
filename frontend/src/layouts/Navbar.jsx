...
const Navbar = () => {
  ...
  const name = user ? JSON.parse(user).fullname : '';
  const groupName = user ? JSON.parse(user).groupName : '';
  return (
    <nav
          ...
          <li className="nav-item navbar-dropdown dropdown-user dropdown">
            <a aria-label='dropdown profile avatar' className="nav-link dropdown-toggle hide-arrow" href="#" data-bs-toggle="dropdown">
              <div className="avatar avatar-online">
                <img src="../assets/img/avatars/empty-avatar.png" className="w-px-40 h-auto rounded-circle" alt="avatar-image" aria-label='Avatar Image' />
              </div>
            </a>
            <ul className="dropdown-menu dropdown-menu-end">
                ...
                <a aria-label='go to profile' className="dropdown-item" href="#">
                  <div className="d-flex">
                    ...
                    <div className="flex-grow-1">
                      <span className="fw-medium d-block">{name}</span>
                      <small className="text-muted">{groupName}</small>
                    </div>
                  </div>
                </a>
              </li>
              ...
              <li>
                <a aria-label='click to log out' className="dropdown-item" onClick={() => {
                  localStorage.clear();
