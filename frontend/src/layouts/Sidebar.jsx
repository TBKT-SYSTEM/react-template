...
const Sidebar = () => {
    ...
    const getMenuItems = async () => {
        try {
            ...
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/sidebars/${groupId}`, {
                headers: {
...
const MenuItem = (item) => {
    ...
    return (
        <li className={`menu-item ${isActive || isSubmenuActive ? 'active' : ''} ${hasSubmenu && isSubmenuActive ? 'open' : ''}`} key={item.id}>
            <NavLink
